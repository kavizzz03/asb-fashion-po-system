...
switch ($action) {
    ...
    case 'searchSuppliers':
        ...
        $data = getSuppliers($search, $perPage, $offset);
    ...
    // ==================== Search Items (with all attributes) ====================
    case 'searchItems':
        ...
        $countSql = str_replace(
            "SELECT i.item_id, i.item_name, i.item_code, i.system_code, d.department_name, sd.sub_department_name, c.category_name, col.color_name, s.size_name, COALESCE(si.cost_price, i.cost_price) as cost_price, COALESCE(si.selling_price, i.selling_price) as selling_price",
    ...
    case 'receivePO':
        ...
        try {
            foreach ($received as $poItemId => $qty) {
                $qty = max(0, (int)$qty);
                $conn->query("UPDATE po_items SET received_qty = received_qty + $qty WHERE po_item_id = $poItemId AND po_id = $poId");
            }
            ...
            if ($remaining == 0) $conn->query("UPDATE po_header SET status = 'Received' WHERE po_id = $poId");
            $conn->commit();
